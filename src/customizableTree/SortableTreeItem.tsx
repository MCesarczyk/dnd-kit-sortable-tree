...
>({
  ...
  const isOverParent = useMemo(
    ...
    [over?.id] // eslint-disable-line react-hooks/exhaustive-deps
  );
  ...
  }, [props.item.id, props.onCollapse]); // eslint-disable-line react-hooks/exhaustive-deps

  const localRemove = useMemo(() => {
    if (!props.onRemove) return undefined;

    return () => props.onRemove?.(props.item.id);
  }, [props.item.id, props.onRemove]); // eslint-disable-line react-hooks/exhaustive-deps
  return (
    <TreeItemComponent
      {...props}
